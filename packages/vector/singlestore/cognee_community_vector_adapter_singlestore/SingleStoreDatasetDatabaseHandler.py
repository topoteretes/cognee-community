...
class SingleStoreDatasetDatabaseHandler(DatasetDatabaseHandlerInterface):
    @classmethod
    async def create_dataset(cls, dataset_id: Optional[UUID], user: Optional[User]) -> dict:
        vector_config = get_vectordb_config()

        if vector_config.vector_db_provider != "singlestore":
            raise ValueError(
                "SingleStoreDatasetDatabaseHandler can only be used with the "
                "SingleStore vector database provider."
            )
